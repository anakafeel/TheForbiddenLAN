...
export class DLS140Client {
  ...
  async getStatus(): Promise<DLS140Status> {
    const res = await fetch(`${this.base}/device/status`, {
  ...
  async getGPS(): Promise<DLS140GPS> {
    const res = await fetch(`${this.base}/location/gps`, {
      headers: this.headers,
    });
    return res.json();
  }
  ...
  async getDataUsage(period: "24h" | "7d" | "all" = "24h") {
    const res = await fetch(`${this.base}/device/data-usage?period=${period}`, {
      headers: this.headers,
    });
    return res.json();
  }
  ...
  async getRoutingPreference(): Promise<{ prefer: "cellular" | "satellite" }> {
    const res = await fetch(`${this.base}/network/routing`, {
      headers: this.headers,
    });
    return res.json();
  }
  ...
  async toSignalStatus(): Promise<SignalStatus> {
    ...
    const activeLink: SignalStatus["activeLink"] =
      status.cellularSignalStrength > 40
        ? "cellular"
